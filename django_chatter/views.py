    ...
    try:
        ...
        try:
            ...
            logger.info(
                ...
                "set it to point to your base template in your settings file.")
            logger.info(
                "django_chatter.views: to turn off this message, set "
                "your settings.CHATTER_DEBUG to False.")
        return 'django_chatter/base.html'


class SessionUserMixin(object):
    @property
    def user(self):
...
class IndexView(LoginRequiredMixin, SessionUserMixin, View):

    def get(self, request, *args, **kwargs):
        rooms_list = Room.objects.filter(members=self.user).order_by('-date_modified')
...
class ChatRoomView(LoginRequiredMixin, SessionUserMixin, TemplateView):
        ...
        if self.user in all_members:
            ...
            for room in rooms_list:
                try:
                    ...
                    continue
                if self.user not in message.recipients.all():
            ...
            raise Http404("Извените! У вас нет искомого диалога.")
...
class ChatUrlView(SessionUserMixin, LoginRequiredMixin, View):
    ...
    def get_room(self, request):
        """
        Use the util room creation function to create room for one/two user(s).
        This can be extended in the future to add multiple users in a group chat.
        """
        user = self.user
